...
module.exports = function(registro, storage){
    ...
    const file_img = storage.file('amcham/' + img_name);
    const tmp_path_img = os.tmpdir() + '/' + registro.key;

    let template;

    return QRCode.toFile(tmp_path_img, [registro.key,].join('\n')).then( () => {
        return Jimp.read('./assets/badge.png');
    }).then((_template) => {
        template = _template;
        ...
        return qrcode.resize(150, 150);
    }).then((qrcode) => {
        return template.composite(qrcode, 450, 175, [Jimp.BLEND_DESTINATION_OVER, 1, 1]).rotate(90).getBufferAsync(Jimp.MIME_PNG);
    }).then((_template) => {
